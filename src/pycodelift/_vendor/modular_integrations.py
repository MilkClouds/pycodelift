...
# pycodelift: switch for transformers' "namespace package" prefix heuristic.
# When True, ``convert_relative_import_to_absolute`` prepends the parent
# directory of *package_name* to absolute import paths whenever that parent
# directory is not literally named ``src`` (the convention used for layouts
# like ``optimum/habana/...`` where the importable name is ``optimum.habana``
# rather than just ``habana``). Default is False — flat packages are the
# common case and prepending would break them. Re-enabled by the
# transformers adapter for exact-fidelity behaviour.
...
def convert_relative_import_to_absolute(
    ...
    # Handle special case where the import comes from a namespace package
    # (e.g. optimum with `optimum.habana`, `optimum.intel` instead of `src.optimum`).
    # pycodelift: gated on the module-level PREPEND_NAMESPACE_PARENT flag so
    # flat packages — which are the default — don't get spurious prefixes.
    if (
        PREPEND_NAMESPACE_PARENT
        and package_name != "transformers"
        and file_parts[pkg_index - 1] != "src"
    ):
