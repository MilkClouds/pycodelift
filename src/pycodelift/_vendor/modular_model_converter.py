...
import glob
import importlib
import logging  # pycodelift: TODO generalize — was `from transformers import logging`
...
# pycodelift: relative imports for sibling vendor modules (was top-level
# imports relying on `utils/` being on sys.path inside transformers).
...
# pycodelift: TODO generalize — Phase 3 replaces this transformers-specific
# table with a pluggable `RenamePolicy.casing_table`. For now an empty dict
# keeps the file importable; transformers-adapter will inject the real one.
CONFIG_MAPPING_NAMES: dict[str, str] = {}

# pycodelift: list of textual call patterns that should be forced to the very
# last position of an ``__init__`` body. transformers' original code
# hard-coded ``"self.post_init("``; the pycodelift core defaults to no
# patterns, and the transformers adapter re-instates the original behaviour
# via ``TrailingCallHook("post_init")``.
TRAILING_METHOD_CALLS: tuple[str, ...] = ()
...
class ReplaceSuperCallTransformer(cst.CSTTransformer):
    ...
    def _fix_post_init_location(self, new_body: list[cst.CSTNode]):
        """Move any trailing-call statements to the very last position.

        pycodelift: the original transformers code hard-coded
        ``"self.post_init("`` here. We now consult the module-level
        :data:`TRAILING_METHOD_CALLS` tuple, which the
        :class:`pycodelift.Flattener` populates from
        :class:`pycodelift.policies.hooks.TrailingCallHook` entries. An
        empty tuple disables this pass entirely.
        """
        patterns = TRAILING_METHOD_CALLS
        if not patterns:
            return new_body
        for i, node in enumerate(new_body):
            ...
            comment_less_code = re.sub(r"#.*", "", code).strip()
            comment_less_code = re.sub(r"\ *\n", "\n", comment_less_code).strip()
            if any(p in comment_less_code for p in patterns) and i < len(new_body) - 1:
...
def find_file_type(class_name: str, model_name: str) -> str:
    ...
    """
    # pycodelift: short-circuit when the suffix table is empty so we don't
    # build an ``re.search(r"()$", ...)`` that vacuously matches and then
    # KeyErrors on ``TYPE_TO_FILE_TYPE[""]``. The empty-table case is the
    # default for framework-agnostic Flattener users — every class lands
    # in the ``modeling`` stem.
    if not TYPE_TO_FILE_TYPE:
        return "modeling"
...
class ModularFileMapper(ModuleMapper):
    ...
    def __init__(self, python_module, new_name, package_name):
        ...
        # pycodelift: fall back to an empty list when the caller's package_name
        # is not registered in EXCLUDED_EXTERNAL_FILES. The original code
        # raised KeyError for any non-transformers package even when no
        # exclusions were needed. Phase 3 replaces this with the
        # `Flattener.excluded_external_files` policy.
        self.excluded_external_files = (
            {} if package_name == "transformers" else EXCLUDED_EXTERNAL_FILES.get(package_name, [])
        )
        # fmt: on
